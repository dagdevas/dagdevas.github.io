...
const AdminProducts = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await productsAPI.getAllAdmin({ page: 1, limit: 100 });
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
        <h3>Продукция</h3>
        <button className="btn" onClick={() => navigate('/admin/products/new')}>Добавить</button>
      </div>
        ...
        <div className="table">
          {items.map(item => (
            <div key={item._id} className="table-row" style={{ display: 'grid', gridTemplateColumns: '240px 1fr 120px', gap: 12, padding: '8px 0', borderBottom: '1px solid #eee' }}>
              ...
              <div>
                <Link className="btn" to={`/admin/products/${item._id}`}>Редактировать</Link>
              </div>
            </div>
          ))}
        </div>
